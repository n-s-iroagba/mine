...
class EmailService extends utils_1.BaseService {
    constructor(url) {
        super('EmailService');
    ...
    async sendEmail(emailData) {
        try {
            ...
            if (typeof emailData.to === 'number') {
                ...
                if (!user) {
                    throw new utils_1.NotFoundError('User');
                }
                ...
                if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailData.to)) {
                    throw new utils_1.ValidationError('Invalid email format');
                }
            ...
            const emailOptions = {
                to: recipientEmail,
                subject: emailData.subject,
                html: emailData.html || this.generateEmailTemplate(emailData?.message, emailData.type),
            };
            const result = await utils_1.EmailHelper.sendEmail(emailOptions);
            if (result) {
    ...
    async sendEmailToAllMiners(subject, message) {
        try {
    ...
    generateEmailTemplate(message, type = 'general') {
